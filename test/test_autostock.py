...
async def test_get_all_stock_code():
    ...
    async with Client(mcp) as client:
        ...
        result = result.data
        print(f"  获取到的股票代码总数: {len(result.get('assert', []))}")
        ...
        result = result.data
        print(f"  搜索到的 '圣泉' 相关的股票代码数量: {len(result.get('assert', []))}")
...
async def test_get_all_index_code():
    ...
    async with Client(mcp) as client:
        ...
        result = result.data
        print(f"  获取到的指数代码总数: {len(result.get('assert', []))}")
    print("-" * 30)
...
async def test_get_stock_industry_code():
    ...
    async with Client(mcp) as client:
        ...
        result = result.data
        print(f"  获取到的股票行业代码总数: {len(result.get('assert', []))}")
        print("-" * 30)
...
async def test_get_stock_rank():
    ...
    async with Client(mcp) as client:
        ...
        result = result.data
        print(f"  获取到的股票排名数据条数: {len(result.get('assert', []))}")
        print("-" * 30)
...
async def test_get_stock_info():
    ...
    async with Client(mcp) as client:
        ...
        result = result.data
        print(f"  获取到的股票详细信息: {len(result.get('assert', []))}")
        print("-" * 30)
...
async def test_get_stock_minute_data():
    ...
    async with Client(mcp) as client:
        ...
        result = result.data
        print(f"  获取到的股票分钟线数据条数: {len(result.get('assert', []))}")
        print("-" * 30)
...
async def test_get_stock_day_kline():
    ...
    async with Client(mcp) as client:
        ...
        result = result.data
        print(f"  获取到的股票日 K 线数据条数 (2025-05-01 to 2025-08-01): {len(result.get('assert', []))}")
        print("-" * 30)
