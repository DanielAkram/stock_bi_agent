...
def _call_api(endpoint: str, params: dict = None):
    ...
    try:
        ...
        if data.get("code") == 200:
            return {"success": True, "message": data.get("message"), "assert": data.get("assert")}
        else:
            return {"success": False, "message": data.get("message", "操作失败"), "assert": None}

    except requests.exceptions.ConnectionError:
        return {"success": False, "message": f"连接错误：无法连接到后端服务 ({BASE_URL})。", "assert": None}
    except requests.exceptions.HTTPError as e:
        return {"success": False, "message": f"API 请求失败：{e}。响应内容: {response.text}", "assert": None}
    except Exception as e:
        return {"success": False, "message": f"发生未知错误：{e}", "assert": None}
